...
import { findCatchableGoldfishId } from './catchGoldfish'

describe('findCatchableGoldfishId', () => {
  it('捕獲半径内に金魚がいれば、そのidを返す', () => {
    const goldfish = [{ id: 1, xPercent: 50, yPercent: 50 }]
    const result = findCatchableGoldfishId({ xVw: 52, yVh: 51 }, goldfish)
    expect(result).toBe(1)
  })

  it('捕獲半径外の金魚は対象にならない', () => {
    const goldfish = [{ id: 1, xPercent: 50, yPercent: 50 }]
    const result = findCatchableGoldfishId({ xVw: 90, yVh: 90 }, goldfish)
    expect(result).toBeNull()
  })

  it('金魚が1匹もいない場合はnullを返す', () => {
    const result = findCatchableGoldfishId({ xVw: 50, yVh: 50 }, [])
    expect(result).toBeNull()
  })
  ...
  it('複数の金魚が半径内にいる場合、最も近い1匹のみを返す', () => {
    ...
    const result = findCatchableGoldfishId({ xVw: 50, yVh: 50 }, goldfish)
