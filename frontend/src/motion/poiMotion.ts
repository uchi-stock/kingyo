...
// および端に張り付いたままになるのを防ぐ）。issue #36で追加した当初の強さ（0.5、毎秒50%減衰）は
// 操作をやめた直後にも目に見えて中央へ戻ってしまうほど強すぎ、issue #39でその反動として
// 弱めすぎた値（0.995）は逆に端へ張り付いたまま戻ってこず操作しにくくなっていた（issue #55）。
// 動かして止めた直後（数秒程度）は大きくは戻らないが、そのまま放置すると数秒〜十数秒程度で
// 中央付近まで寄っていく、両者の中間程度の強さに調整している。
const RECENTER_DECAY_PER_SEC = 0.9
...
// この復元力は、動かして止めた直後（数秒程度）には大きく戻らないが、そのまま放置すると
// 数秒〜十数秒程度で中央付近まで寄っていく強さに調整している（issue #39, #55）。
// そのため残留バイアスへの耐性は限定的で、実機で同種のドリフトが再発した場合は、
// 位置側で後から補正するのではなく removeGravity と同様に加速度側で残留バイアスを
// 推定・除去する方式を検討する。
